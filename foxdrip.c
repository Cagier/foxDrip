...
XDATA void print_32asHex(uint32 valToPrint) {
//	uint32 actualVal;
	uint16 valByte1;
	uint16 valByte2;
	valByte1 = valToPrint >> 16;
	valByte2 = valToPrint & 0xffff;
    printf("%01X", valByte1);
    printf("%04X", valByte2);
}

XDATA void print_packet(Dexcom_packet* pPkt) {
//        Send a Sigfox frame in hexadecimal format using the AT$SF command
//        The payload will be configured in the Sigfox backend to automatically populate the mlab database using the mlab REST API
	...
	printf("AT$SF=");
	printf("%.2X", pPkt->txId);								// Byte 1

	for(i = 0; i < 5; ++i)									// Bytes 2-6
		printf("%.2X", transID[i]);

    print_32asHex(dex_num_decoder(pPkt->raw));				// Bytes 7-9
    print_32asHex((dex_num_decoder(pPkt->filtered))*2);		// Bytes 9-11
	printf("%.2X", pPkt->battery);							// Byte 12
	printf("\r\n");											// CR+LF
